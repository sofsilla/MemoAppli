...
jsPsych.plugins['survey-order-image'] = (function() {
  var plugin = {};


  plugin.info = {
    name: 'survey-order-image',
    description: '',
    parameters: {
      questions: {
  ...
  plugin.trial = function(display_element, trial) {
    var plugin_id_name = "jspsych-survey-order-image";
    ...
    display_element.innerHTML = '<style id="jspsych-survey-order-image-css"></style>';
    var cssstr = ".jspsych-survey-order-image-question { margin-top: 2em; margin-bottom: 2em; text-align: left; }"+
      ".jspsych-survey-order-image-text span.required {color: darkred;}"+
      ".jspsych-survey-order-image-horizontal .jspsych-survey-order-image-text {  text-align: center;}"+
      ".jspsych-survey-order-image-option { line-height: 2; }"+
      ".jspsych-survey-order-image-horizontal .jspsych-survey-order-image-option {  display: inline-block;  margin-left: 1em;  margin-right: 1em;  vertical-align: top;}"+
      "label.jspsych-survey-order-image-text input[type='checkbox'] {margin-right: 1em;}"+
    ...
    display_element.querySelector('#jspsych-survey-order-image-css').innerHTML = cssstr;
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      // add question text
      display_element.querySelector(question_selector).innerHTML += '<p id="survey-question" class="' + plugin_id_name + '-text survey-order-image">' + trial.questions[i].prompt + '</p>';
...
function validate_order_img(element){
  var img = element.nextElementSibling.firstChild;
   element.checked ? img.classList.add("selected_recall_item") : img.classList.remove("selected_recall_item");
   item_number ++
}