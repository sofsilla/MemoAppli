...
jsPsych.plugins['survey-multi-choice-image'] = (function() {
  ...
  if(trial.keyboard_voice){
      ...
      var input_speech = document.createElement("INPUT");
      input_speech.setAttribute('id', 'free_recall_input');
      input_speech.setAttribute("type", "text");


      var a_speech = document.createElement("A");
      a_speech.setAttribute('href', '#');
      ...
      if (isChrome)
        img_speech.setAttribute("onclick", "startListening()") ;
      a_speech.appendChild(img_speech);

      div_speech.appendChild(input_speech);
      div_speech.appendChild(a_speech);
    ...
    trial_form.addEventListener('submit', function(event) {
      ...
      for(var i=0; i<matches.length; i++){
        ...
        if(trial.keyboard_voice){
            var val = "";
            if ($("#free_recall_input").val() != "" ) {
              val = "res/img/"+fold + $("#free_recall_input").val().replace(/\s+$/, '') + ".png" ;
            }
        }else{
